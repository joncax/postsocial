...
from storage.models import Queue, Platform
...
async def list_queues(db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(Queue).order_by(Queue.created_at.desc()))
...
async def update_queue(queue_id: UUID, data: QueueUpdate, db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(Queue).where(Queue.id == queue_id))
    queue = result.scalar_one_or_none()
    if not queue:
        raise HTTPException(status_code=404, detail="Fila não encontrada")
